...
console.log(copiedId)
  return (
    ...
    <div onClick={()=> setIsHovered(!isHovered)} style={{opacity: isHovered ? 0 : 1, zIndex: isHovered ? -1 : 1}} className='absolute inset-0 z-20 rounded-2xl opacity-100 group-hover/outer:opacity-0 group-active/outer:opacity-0 group-hover/outer:-z-10 group-active/outer:-z-10 transition-opacity duration-500' style={{backgroundImage: bookmark.ogImage ? `url(${bookmark.ogImage})` : '', backgroundSize: '100% 100%', backgroundPosition: 'center', backgroundRepeat: 'no-repeat'}}></div>
