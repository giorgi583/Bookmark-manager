...
const [searchQuery, setSearchQuery] = useState<string>('');
  ...
  return (
    <div className="flex flex-col gap-4 border-2 border-gray-300 p-4 rounded-xl min-w-200 relative">
      <BookmarkIcon size={46} className="absolute -top-2 right-5 text-lime-100" fill='lime'  />
  ...
  {  showedBookmarks === 'Favourites' ? favorites.length > 0 ? favorites.filter((bookmark: any) => bookmark.title.toLowerCase().includes(searchQuery.toLowerCase().trim()) || bookmark.tags.some((tag: string) => tag.toLowerCase().includes(searchQuery.toLowerCase().trim()))).map((bookmark: any) => <Bookmark key={bookmark._id} bookmark={bookmark} copiedId={copiedId} setCopiedId={setCopiedId} />) 
  : <p className="p-4 bg-taupe-600/20 rounded-2xl flex justify-between items-center gap-4 text-2xl py-20">No bookmarks found!</p> 
  : bookmarks.length > 0 ? bookmarks.filter((bookmark: any) => {return bookmark.title.toLowerCase().includes(searchQuery.toLowerCase().trim()) || bookmark.tags.some((tag: string) => tag.toLowerCase().includes(searchQuery.toLowerCase().trim()))}).map((bookmark: any) => <Bookmark key={bookmark._id} bookmark={bookmark} copiedId={copiedId} setCopiedId={setCopiedId} />) 
